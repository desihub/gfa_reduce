...
def _check_header_cards(header):
    ...
    for kw in keywords:
        if (kw not in header) or (header[kw] is None) or (type(header[kw]).__name__ == 'Undefined'):
            log.warning('SKIPPING DESIMETER FIELD MODEL DUE TO MISSING METADATA')
            return False
...
def fit_dm_fieldmodel(header, ccds, _catalog, check_header_cards=False):
    ...
    if len(np.unique(catalog['extname'])) < 2:
        log.warning('not enough sources with Gaia counterparts for a FieldModel')
        return None
    ...
    for kw in ['ADC1PHI', 'ADC2PHI']:
        if kw not in header:
            log.warning(kw + ' missing !!! assuming ' + kw + ' = 0.0')
    ...
    fm.desimeter_gitrev = retrieve_git_rev(fname=desimeter.__file__)

    dt = time.time()-t0
    log.info('total time taken to derive desimeter FieldModel : ', dt, ' seconds')
    return fm
